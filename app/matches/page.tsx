...
import { MatchRequestSchema } from '../../src/types/user'
...
export default function OpportunitiesPage() {
  ...
  const fetchOpportunities = async () => {
    try {
      ...
      const response_settings = await fetch('http://localhost:3000/api/user', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      })
      const settings = await response_settings.json()
      console.log('settings', settings)
      const parsed = MatchRequestSchema.parse(settings.data)
